...
import static com.gllue.myproxy.common.util.SQLStatementUtils.getAliasOrTableName;
import static com.gllue.myproxy.common.util.SQLStatementUtils.newExprTableSource;
import static com.gllue.myproxy.common.util.SQLStatementUtils.newSQLSelectItems;
...
import com.gllue.myproxy.metadata.model.TemporaryTableMetaData;
...
public class BaseSelectQueryRewriteVisitor extends MySqlASTVisitorAdapter {
  ...
  public boolean visit(SQLPropertyExpr x) {
    ...
    if (table != null) {
      if (table.getType() == TableType.TEMPORARY) {
        ...
        if (TablePartitionHelper.EXTENSION_TABLE_ID_COLUMN.equals(columnName)) {
          return false;
  ...
  private void rewriteSelectAllColumnExpr(List<SQLSelectItem> items, List<Object> tableAliases) {
    ...
    for (var element : tableAliases) {
      if (element instanceof String) {
        newItems.add(new SQLSelectItem(new SQLPropertyExpr((String) element, ServerConstants.ALL_COLUMN_EXPR)));
      } else if (element instanceof SQLExpr) {
        newItems.add(new SQLSelectItem(new SQLPropertyExpr((SQLExpr) element, ServerConstants.ALL_COLUMN_EXPR)));
      }
    }
  ...
  protected void rewriteSingleTableSelectAllColumnExpr(List<SQLSelectItem> items) {
    ...
    for (var item : items) {
      ...
      if (!ServerConstants.ALL_COLUMN_EXPR.equals(expr.getName())) {
        continue;
      }
  ...
  private SQLBinaryOpExpr newTableJoinCondition(
    ...
    var left = new SQLPropertyExpr(primaryTableAlias, TablePartitionHelper.QUOTED_EXTENSION_TABLE_ID_COLUMN);
    var right = new SQLPropertyExpr(extensionTableAlias, TablePartitionHelper.QUOTED_EXTENSION_TABLE_ID_COLUMN);
  ...
  private TemporaryTableMetaData resolveSubQuerySelectColumns(
    ...
    for (var item : items) {
      ...
      if (expr instanceof SQLPropertyExpr) {
        ...
        if (ServerConstants.ALL_COLUMN_EXPR.equals(columnName)) {
          var table = scope.getTable(schema, tableOrAlias);
          if (table == null) {
