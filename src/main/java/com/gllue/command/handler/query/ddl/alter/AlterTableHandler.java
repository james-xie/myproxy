...
import com.gllue.metadata.command.UpdatePartitionTableCommand;
import com.gllue.metadata.command.UpdatePartitionTableCommand.Table;
import com.gllue.metadata.command.UpdateTableCommand;
...
public class AlterTableHandler extends AbstractDDLHandler {
  ...
  private String applyChangesToColumnsMap(
      String tableName,
      Map<String, AbstractTableUpdateCommand.Column> columnsMap,
      SQLAlterTableStatement stmt) {
  ...
  private boolean updatePartitionTableMetaData(
    ...
    for (var stmt : alterTableStmtList) {
      alterTableStmtMap.put(unquoteName(stmt.getTableName()), stmt);
    }
